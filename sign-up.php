...
if (!empty($_POST)) {
    ...
    if (isset($_FILES['img'])) {
        ...
        if (!(in_array($file_extension, $white_list))) {
            $data['errors']['img'] = 'Загрузите фото в формате "jpg", "jpeg" или "png"';
        } elseif (move_uploaded_file($file['tmp_name'], $file_name)) {
            $data['new_user']['avatar'] = $file_name;
        } else {
            $data['errors']['img'] = 'Возникла ошибка при загрузке файла';
        }
    } else {
...
<?= includeTemplate('templates/footer.php', ['categories' => $data['categories']]) ?>
